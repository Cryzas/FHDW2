create or replace package SrvrFacade is
    ...
    function currCompsGet(SrvrId number) return ManyServices.ManyInfoCollection;
    function currCompsAdd(SrvrId number, currCompsVal number, currCompsCls number) return number;
    procedure currCompsRem(currCompsId number);

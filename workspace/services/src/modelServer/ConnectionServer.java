...
import persistence.Administrator4Public;
import persistence.PersistentServer;
import persistence.Server4Public;
import persistence.PersistentCustomer;
import persistence.Customer4Public;
import persistence.PersistentRegister;
import persistence.Register4Public;
import persistence.PersistentUserManager;
import persistence.UserManager4Public;
...
public class ConnectionServer extends RemoteServerMaster {
	...
	public RemoteServerMaster createRemoteServer(String connectionName, String userName, long objectId, long classId){
		try {
			if(classId == -114)return new RemoteAdministrator(connectionName, userName, (PersistentAdministrator)PersistentProxi.createProxi(objectId, classId));
			...
			if(classId == -115)return new RemoteCustomer(connectionName, userName, (PersistentCustomer)PersistentProxi.createProxi(objectId, classId));
			if(classId == -118)return new RemoteRegister(connectionName, userName, (PersistentRegister)PersistentProxi.createProxi(objectId, classId));
			if(classId == -116)return new RemoteUserManager(connectionName, userName, (PersistentUserManager)PersistentProxi.createProxi(objectId, classId));

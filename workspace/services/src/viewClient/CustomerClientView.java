...
public class CustomerClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private CustomerView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public CustomerClientView( ExceptionAndEventHandler parent, CustomerView service) {
		super();
		this.parent = parent;
	...
	private CustomerView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getCustomerView();
		if (object == null){
	...
	public CustomerConnection getConnection(){
        	return (CustomerConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getCustomerView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getCustomerView());	
    ...
    interface MenuItemVisitor{
        ImageView handle(CustomerOpPRMTRMenuItem menuItem);
    }
    private abstract class CustomerMenuItem extends MenuItem{
        private CustomerMenuItem(){
            this.setGraphic(getIconForMenuItem(this));
        }
        abstract protected ImageView accept(MenuItemVisitor visitor);
    }
    private class CustomerOpPRMTRMenuItem extends CustomerMenuItem{
    ...
    private java.util.Vector<javafx.scene.control.Button> getToolButtonsForStaticOperations() {
        ...
        currentButton = new javafx.scene.control.Button("customerOp");
        currentButton.setGraphic(new CustomerOpPRMTRMenuItem().getGraphic());
        currentButton.setOnAction(new EventHandler<ActionEvent>(){
            public void handle(javafx.event.ActionEvent e) {
                Alert confirm = new Alert(AlertType.CONFIRMATION);
                confirm.setTitle(GUIConstants.ConfirmButtonText);
                confirm.setHeaderText(null);
                confirm.setContentText("customerOp" + GUIConstants.ConfirmQuestionMark);
                confirm.setX( getPointForView().getX() );
                confirm.setY( getPointForView().getY() );
                Optional<ButtonType> buttonResult = confirm.showAndWait();
                if (buttonResult.get() == ButtonType.OK) {
                    try {
                        getConnection().customerOp();
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        MenuItem item = null;
        item = new CustomerOpPRMTRMenuItem();
        item.setText("(S) customerOp");
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().customer_Menu_Filter((Anything)selected));
            } catch (ModelException me){
	...
	private ImageView getIconForMenuItem(CustomerMenuItem menuItem){

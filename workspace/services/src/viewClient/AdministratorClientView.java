...
public class AdministratorClientView extends BorderPane implements ExceptionAndEventHandler{
	...
	**/
    ...
    interface MenuItemVisitor{
        ImageView handle(AdministratorOpPRMTRMenuItem menuItem);
    }
    private abstract class AdministratorMenuItem extends MenuItem{
        private AdministratorMenuItem(){
            this.setGraphic(getIconForMenuItem(this));
        }
        abstract protected ImageView accept(MenuItemVisitor visitor);
    }
    private class AdministratorOpPRMTRMenuItem extends AdministratorMenuItem{
    ...
    private java.util.Vector<javafx.scene.control.Button> getToolButtonsForStaticOperations() {
        ...
        currentButton = new javafx.scene.control.Button("administratorOp");
        currentButton.setGraphic(new AdministratorOpPRMTRMenuItem().getGraphic());
        currentButton.setOnAction(new EventHandler<ActionEvent>(){
            public void handle(javafx.event.ActionEvent e) {
                Alert confirm = new Alert(AlertType.CONFIRMATION);
                confirm.setTitle(GUIConstants.ConfirmButtonText);
                confirm.setHeaderText(null);
                confirm.setContentText("administratorOp" + GUIConstants.ConfirmQuestionMark);
                confirm.setX( getPointForView().getX() );
                confirm.setY( getPointForView().getY() );
                Optional<ButtonType> buttonResult = confirm.showAndWait();
                if (buttonResult.get() == ButtonType.OK) {
                    try {
                        getConnection().administratorOp();
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        MenuItem item = null;
        item = new AdministratorOpPRMTRMenuItem();
        item.setText("(S) administratorOp");
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().administrator_Menu_Filter((Anything)selected));
            } catch (ModelException me){
	...
	private ImageView getIconForMenuItem(AdministratorMenuItem menuItem){

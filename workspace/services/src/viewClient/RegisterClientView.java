...
public class RegisterClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private RegisterView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public RegisterClientView( ExceptionAndEventHandler parent, RegisterView service) {
		super();
		this.parent = parent;
	...
	private RegisterView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getRegisterView();
		if (object == null){
	...
	public RegisterConnection getConnection(){
        	return (RegisterConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getRegisterView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getRegisterView());	
    ...
    private abstract class RegisterMenuItem extends MenuItem{
        private RegisterMenuItem(){
            this.setGraphic(getIconForMenuItem(this));
        }
        abstract protected ImageView accept(MenuItemVisitor visitor);
    }
    private class RegisterPRMTRStringPRMTRStringPRMTRMenuItem extends RegisterMenuItem{
    ...
    private java.util.Vector<javafx.scene.control.Button> getToolButtonsForStaticOperations() {
        ...
        currentButton.setOnAction(new EventHandler<ActionEvent>(){
            public void handle(javafx.event.ActionEvent e) {
                final RegisterRegisterStringStringMssgWizard wizard = new RegisterRegisterStringStringMssgWizard("register");
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().register_Menu_Filter((Anything)selected));
	...
	class RegisterRegisterStringStringMssgWizard extends Wizard {

		protected RegisterRegisterStringStringMssgWizard(String operationName){
			super(RegisterClientView.this);
			getOkButton().setText(operationName);
			getOkButton().setGraphic(new RegisterPRMTRStringPRMTRStringPRMTRMenuItem ().getGraphic());
		}
		protected void initialize(){
			this.helpFileName = "RegisterRegisterStringStringMssgWizard.help";
			super.initialize();		
		}
	...
	private ImageView getIconForMenuItem(RegisterMenuItem menuItem){

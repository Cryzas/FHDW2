...
public class StudyGroup extends PersistentObject implements PersistentStudyGroup {
	...
	public static StudyGroup4Public createStudyGroup(String name, boolean delayed$Persistence)
		...
		if (delayed$Persistence) {
			result = ConnectionHandler.getTheConnectionHandler().theStudyGroupFacade.newDelayedStudyGroup(name);
			result.setDelayed$Persistence(true);
		} else {
			result = ConnectionHandler.getTheConnectionHandler().theStudyGroupFacade.newStudyGroup(name, -1);
		}
	...
	public static StudyGroup4Public createStudyGroup(String name, boolean delayed$Persistence, StudyGroup4Public This)
			throws PersistenceException {
		if (name == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		PersistentStudyGroup result = null;
	...
	public java.util.HashMap<String, Object> toHashtable(java.util.HashMap<String, Object> allResults, int depth,
			int essentialLevel, boolean forGUI, boolean leaf, boolean inDerived) throws PersistenceException {
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			if (program != null) {
				String proxiInformation = SearchListRoot.calculateProxiInfoAndRecursiveGet(program, allResults, depth,
						essentialLevel, forGUI, false, essentialLevel <= 1, inDerived, false, false);
				result.put("program", proxiInformation);

			}
			result.put("students", this.getStudents().getVector(allResults, depth, essentialLevel, forGUI, true, true,
					inDerived, false, false));
			AbstractPersistentRoot finished = (AbstractPersistentRoot) this.getFinished();
			if (finished != null) {
				String proxiInformation = SearchListRoot.calculateProxiInfoAndRecursiveGet(finished, allResults, depth,
						essentialLevel, forGUI, false, essentialLevel <= 1, inDerived, false, false);
	...
	public static StudyGroupSearchList getStudyGroupByName(String name) throws PersistenceException {
		return ConnectionHandler.getTheConnectionHandler().theStudyGroupFacade.getStudyGroupByName(name);
	}

	public StudyGroup provideCopy() throws PersistenceException {
		StudyGroup result = this;
		result = new StudyGroup(this.name, this.program, this.finished, this.This, this.getId());
	...
	public StudyGroup(String name, PersistentProgramSGroup program, PersistentMyBoolean finished,
			PersistentStudyGroup This, long id) throws PersistenceException {
		/*
		 * Shall not be used by clients for object construction! Use static create
		 * operation instead!
		 */
		super(id);
		this.name = name;
		...
		if (This != null && !(this.isTheSameAs(This)))
			this.This = This;
	}
	...
	public void store() throws PersistenceException {
		...
		if (this.getClassId() == 180)
			ConnectionHandler.getTheConnectionHandler().theStudyGroupFacade.newStudyGroup(name, this.getId());
		super.store();
	...
	public void setName(String newValue) throws PersistenceException {
		if (newValue == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		if (!this.isDelayed$Persistence())
			ConnectionHandler.getTheConnectionHandler().theStudyGroupFacade.nameSet(this.getId(), newValue);
		this.name = newValue;
	}
	...
	public void setProgram(ProgramSGroup4Public newValue) throws PersistenceException {
		...
		if (newValue.isTheSameAs(this.program))
			return;
	...
	public void setFinished(MyBoolean4Public newValue) throws PersistenceException {
		if (newValue == null)
			throw new PersistenceException("Null values not allowed!", 0);
		if (newValue.isTheSameAs(this.finished))
			return;
		long objectId = newValue.getId();
		long classId = newValue.getClassId();
	...
	protected void setThis(PersistentStudyGroup newValue) throws PersistenceException {
		...
		if (newValue.isTheSameAs(this.This))
			return;
		long objectId = newValue.getId();
		long classId = newValue.getClassId();
	...
	public PersistentStudyGroup getThis() throws PersistenceException {
		if (this.This == null) {
			PersistentStudyGroup result = (PersistentStudyGroup) PersistentProxi.createProxi(this.getId(),
					this.getClassId());
			result.getTheObject();
			return result;
		}
	...
	public <E extends model.UserException> void accept(AnythingExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleStudyGroup(this);
	}

	public <R, E extends model.UserException> R accept(AnythingReturnExceptionVisitor<R, E> visitor)
			throws PersistenceException, E {
		return visitor.handleStudyGroup(this);
	}
	...
	public int getLeafInfo() throws PersistenceException {
		if (this.getProgram() != null)
			return 1;
		if (this.getStudents().getLength() > 0)
			return 1;
		return 0;
	}
	...
	public void endStudyGroup() throws model.AlreadyFinishedException, PersistenceException {

...
public class StudyGroupManagerFacade{
    ...
    public StudyGroupManager getStudyGroupManager(long StudyGroupManagerId) throws PersistenceException{
        try{
            ...
            if (obj.getLong(4) != 0)
                This = (PersistentStudyGroupManager)PersistentProxi.createProxi(obj.getLong(4), obj.getLong(5));
            StudyGroupManager result = new StudyGroupManager(subService,
                                                             This,
                                                             StudyGroupManagerId);
            obj.close();
            callable.close();
    ...
    public void subServiceSet(long StudyGroupManagerId, SubjInterface subServiceVal) throws PersistenceException {
        try{
            CallableStatement callable;
            callable = this.con.prepareCall("Begin " + this.schemaName + ".StdGrpMngrFacade.sbSrvcSet(?, ?, ?); end;");

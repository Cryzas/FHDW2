...
public class StudentFacade{
    ...
    public Student getStudent(long StudentId) throws PersistenceException{
        try{
            ...
            if (obj.getLong(9) != 0)
                This = (PersistentStudent)PersistentProxi.createProxi(obj.getLong(9), obj.getLong(10));
    ...
    public void subServiceSet(long StudentId, SubjInterface subServiceVal) throws PersistenceException {
        try{
            CallableStatement callable;
            callable = this.con.prepareCall("Begin " + this.schemaName + ".StdntFacade.sbSrvcSet(?, ?, ?); end;");
            callable.setLong(1, StudentId);

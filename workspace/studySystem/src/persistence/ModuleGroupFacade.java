...
public class ModuleGroupFacade{
    ...
    public ModuleGroupSearchList inverseGetModules(long objectId, long classId)throws PersistenceException{
        try{
            CallableStatement callable;
            callable = this.con.prepareCall("Begin ? := " + this.schemaName + ".MdlGrpFacade.iGetMdls(?, ?); end;");
            callable.registerOutParameter(1, oracle.jdbc.OracleTypes.CURSOR);
            ...
            while (list.next()) {
                if (list.getLong(3) != 0) result.add((PersistentModuleGroup)PersistentProxi.createProxi(list.getLong(3), list.getLong(4)));
                else result.add((PersistentModuleGroup)PersistentProxi.createProxi(list.getLong(1), list.getLong(2)));
            }
            list.close();
            callable.close();
            return result;

...
public class ProgramFacade{
    ...
    public ProgramSearchList inverseGetModules(long objectId, long classId)throws PersistenceException{
        ...
        while (candidates.hasNext()){
            PersistentProgram current = (PersistentProgram)((PersistentRoot)candidates.next()).getTheObject();
            if (current != null){
                java.util.Iterator<ModuleAbstract4Public> iterator = ((Program_ModulesProxi)current.getModules()).iterator();
                while(iterator.hasNext()){
                    ...
                    if (!item.isDltd() && !current.isDelayed$Persistence() && item.getClassId() == classId && item.getId() == objectId) {
                        PersistentProgram proxi = (PersistentProgram)PersistentProxi.createProxi(current.getId(), current.getClassId());
                        result.add((PersistentProgram)proxi.getThis());
                    }
                }
            }
        }

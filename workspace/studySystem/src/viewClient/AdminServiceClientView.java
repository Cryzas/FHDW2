...
public class AdminServiceClientView extends BorderPane implements ExceptionAndEventHandler {
	...
	private TreeRefresh getNavigationTree() {
		if (this.navigationTree == null) {
			...
			this.navigationTree.getSelectionModel().selectedItemProperty()
					.addListener(new ChangeListener<TreeItem<ViewObjectInTree>>() {
						public void changed(ObservableValue<? extends TreeItem<ViewObjectInTree>> observable,
								TreeItem<ViewObjectInTree> oldValue, TreeItem<ViewObjectInTree> newValue) {
							if (menu != null)
								menu.hide();
							if (newValue != null) {
	...
	protected void setDetailsTable(Anything object) {
		...
		if (object == null && this.getConnection() != null)
			object = this.getConnection().getAdminServiceView();
		if (object == null) {
	...
	protected void tryShowContextMenu(ContextMenuEvent e, TreeRefresh tree, boolean withStaticOperations) {
		...
		this.menu = this.getContextMenu(selected, withStaticOperations,
				new Point2D((float) e.getScreenX(), (float) e.getScreenY()));
	...
	public void initializeConnection() {
		...
		try {
			java.util.Iterator<subAdminServiceView> serviceIterator = this.getConnection().getAdminServiceView()
					.getServices().iterator();
			while (serviceIterator.hasNext()) {
				subAdminServiceView current = serviceIterator.next();
				current.accept(new subAdminServiceVisitor() {

					@Override
					public void handleUserManagerService(UserManagerServiceView userManagerService)
							throws ModelException {
						UserManagerServiceClientView view = new UserManagerServiceClientView(AdminServiceClientView.this, userManagerService);
						userManagerService.connectUserManagerService(AdminServiceClientView.this.getConnection(), view);
						Tab newTab = new Tab(userManagerService.toString(), view);
					...
					public void handleProgramModuleService(ProgramModuleServiceView programModuleService)
						...
						programModuleService.connectProgramModuleService(AdminServiceClientView.this.getConnection(), view);
						Tab newTab = new Tab(programModuleService.toString(), view);
						getMainTabPane().getTabs().add(newTab);
					}

					@Override
					public void handleStudentManageService(StudentManageServiceView studentManageService)
							throws ModelException {
						StudentManageServiceClientView view = new StudentManageServiceClientView(AdminServiceClientView.this, studentManageService);
						studentManageService.connectStudentManageService(AdminServiceClientView.this.getConnection(), view);
						Tab newTab = new Tab(studentManageService.toString(), view);
						getMainTabPane().getTabs().add(newTab);
					}

					@Override
					public void handleStudyGroupService(StudyGroupServiceView studyGroupService) throws ModelException {
						StudyGroupServiceClientView view = new StudyGroupServiceClientView(AdminServiceClientView.this, studyGroupService);
						studyGroupService.connectStudyGroupService(AdminServiceClientView.this.getConnection(), view);
						Tab newTab = new Tab(studyGroupService.toString(), view);
						getMainTabPane().getTabs().add(newTab);
					}
				});
			}
	...
	private javafx.geometry.Point2D getPointForView() {
		javafx.geometry.Point2D result = getNavigationTree().localToScene(
				getNavigationTree().getWidth() / 18 + getNavigationTree().getScene().getWindow().getX()
						+ getNavigationTree().getScene().getX(),
				getNavigationTree().getHeight() / 18 + getNavigationTree().getScene().getWindow().getY()
						+ getNavigationTree().getScene().getY());
		return result;
	}

...
public class ServerClientView extends BorderPane implements ExceptionAndEventHandler{
	...
	**/
	...
	//	this.setCenter(this.getMainTabPane());
		try {
			this.getConnection().getServerView().getService().accept(new ServiceVisitor() {
				
				@Override
				public void handleStudentService(StudentServiceView studentService) throws ModelException {
					StudentServiceClientView view = new StudentServiceClientView(ServerClientView.this, studentService);
					studentService.connectStudentService(ServerClientView.this.getConnection(), view);

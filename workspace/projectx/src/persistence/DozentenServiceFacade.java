...
public class DozentenServiceFacade{
    ...
    public DozentenService getDozentenService(long DozentenServiceId) throws PersistenceException{
        try{
            ...
            if (obj.getLong(4) != 0)
                programManager = (PersistentProgramManager)PersistentProxi.createProxi(obj.getLong(4), obj.getLong(5));
            PersistentModuleManager moduleManager = null;
            if (obj.getLong(6) != 0)
                moduleManager = (PersistentModuleManager)PersistentProxi.createProxi(obj.getLong(6), obj.getLong(7));
            PersistentStudyGroupManager groupManager = null;
            if (obj.getLong(8) != 0)
                groupManager = (PersistentStudyGroupManager)PersistentProxi.createProxi(obj.getLong(8), obj.getLong(9));
            PersistentStudentManager studentManager = null;
            if (obj.getLong(10) != 0)
                studentManager = (PersistentStudentManager)PersistentProxi.createProxi(obj.getLong(10), obj.getLong(11));
            DozentenService result = new DozentenService(This,
                                                         programManager,
                                                         moduleManager,
                                                         groupManager,
                                                         studentManager,

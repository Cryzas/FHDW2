...
public class Server extends PersistentObject implements PersistentServer {
	...
	public static Server4Public createServer(String password, String user, long hackCount, java.sql.Timestamp hackDelay)
	...
	public static Server4Public createServer(String password, String user, long hackCount, java.sql.Timestamp hackDelay,
			boolean delayed$Persistence, Server4Public This) throws PersistenceException {
		if (password == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
		if (user == null)
			throw new PersistenceException("Null not allowed for persistent strings, since null = \"\" in Oracle!", 0);
	...
	public java.util.HashMap<String, Object> toHashtable(java.util.HashMap<String, Object> allResults, int depth,
		...
		if (depth > 0 && essentialLevel <= common.RPCConstantsAndServices.EssentialDepth) {
			...
			result.put("currentComponents", this.getCurrentComponents().getVector(allResults, depth, essentialLevel,
					forGUI, tdObserver, false, true));
			result.put("errors",
					this.getErrors().getVector(allResults, depth, essentialLevel, forGUI, tdObserver, false, true));
	...
	public <E extends model.UserException> void accept(InvokerExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleServer(this);
	}

	public <R, E extends model.UserException> R accept(InvokerReturnExceptionVisitor<R, E> visitor)
			throws PersistenceException, E {
		return visitor.handleServer(this);
	}
	...
	public <E extends model.UserException> void accept(RemoteExceptionVisitor<E> visitor)
			throws PersistenceException, E {
		visitor.handleServer(this);
	}

	public <R, E extends model.UserException> R accept(RemoteReturnExceptionVisitor<R, E> visitor)
			throws PersistenceException, E {
		return visitor.handleServer(this);
	}

	public int getLeafInfo() throws PersistenceException {
		if (this.getCurrentComponents().getLength() > 0)
			return 1;
		return 0;
	}
	...
	public String server_Menu_Filter(final Anything anything) throws PersistenceException {
		String result = "+++";
		return result;
	}
	...
	public void clearComponents() throws PersistenceException {
	...
	public void createMaterial(final String name) throws model.PartsListException, PersistenceException {
		if (Component.getComponentByName(name).iterator().hasNext()) {
			throw new PartsListException("Komponente existiert bereits");
		}
		getThis().getCurrentComponents().add(Material.createMaterial(name));
		getThis().signalChanged(true);

	}

	public void createProduct(final String name) throws model.PartsListException, PersistenceException {
		if (Component.getComponentByName(name).iterator().hasNext()) {
			throw new PartsListException("Komponente existiert bereits");
		}
		getThis().getCurrentComponents().add(Product.createProduct(name));
		getThis().signalChanged(true);
	...
	public ComponentLst4Public fetchMaterials(final Component4Public component) throws PersistenceException {
		return component.fetchMaterials();
	}

	public void findComponents(final String name) throws PersistenceException {

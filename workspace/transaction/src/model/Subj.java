...
public class Subj extends PersistentObject implements PersistentSubj{
    ...
    public static Subj4Public getById(long objectId) throws PersistenceException{
        long classId = ConnectionHandler.getTheConnectionHandler().theSubjFacade.getClass(objectId);
        return (Subj4Public)PersistentProxi.createProxi(objectId, classId);
    }
    
    public static Subj4Public createSubj() throws PersistenceException{
        return createSubj(false);
    }
    
    public static Subj4Public createSubj(boolean delayed$Persistence) throws PersistenceException {
    ...
    public static Subj4Public createSubj(boolean delayed$Persistence,Subj4Public This) throws PersistenceException {
        PersistentSubj result = null;
        if(delayed$Persistence){
            result = ConnectionHandler.getTheConnectionHandler().theSubjFacade
                .newDelayedSubj();
    ...
    public Subj(PersistentSubj This,long id) throws PersistenceException {
    ...
    public void store() throws PersistenceException {
        ...
        if (this.getClassId() == 130) ConnectionHandler.getTheConnectionHandler().theSubjFacade
            .newSubj(this.getId());
        super.store();
    ...
    public PersistentSubj getThis() throws PersistenceException {
        if(this.This == null){
            PersistentSubj result = (PersistentSubj)PersistentProxi.createProxi(this.getId(),this.getClassId());
            result.getTheObject();
            return result;
    ...
    public synchronized void deregister(final ObsInterface observee) 
		...
		while (observers.hasNext()){
			...
			if (current.isTheSameAs(observee))observers.remove();
		}
    }
